...
export function PersonDiscoursePanel({
  ...
  const codedInWindow = monthly.reduce((acc, m) => acc + m.codedTotal, 0);
  const stats = computeRateStats(monthly, framework);
  const visibleDots =
    selectedYear === null
      ? speechDots
      : speechDots.filter((d) => d.sessionDate.startsWith(String(selectedYear)));
  const yearRange = { first: firstActiveYear, last: lastActiveYear };
  return (
    <section className={cn(className)} aria-labelledby="discurs-analiza">
      <SectionOpener
        id="discurs-analiza"
        eyebrow="III"
        title="Analiza discursului"
        dek="Codări Hawkins, V-Party, DQI și voce. Cifrele descriu speech-acturi, nu trăsături personale."
        aside={
          <>
            <VoiceToggle
              basePath={basePath}
        ...
        {framework === "voice" ? (
          ...
          <div className="space-y-3">
            ...
            <SpeechDotScatter
              dots={visibleDots}
              year={selectedYear}
              firstYear={firstActiveYear}
              lastYear={lastActiveYear}
              axis={framework === "dqi" ? "dqi" : framework}
            />
            {topMarkerKinds.length > 0 && framework !== "dqi" ? (
              <div className="space-y-1">
                <p className="label-mono text-ink-30">
                  Marcheri principali{" "}
                  {selectedYear === null ? "în întreaga carieră" : `în ${selectedYear}`}
                </p>
                <div className="flex flex-wrap gap-1">
                  ...
                  {topMarkerKinds.filter((m) => m.framework === framework).length === 0 ? (
                    <span className="text-xs text-ink-45">
                      Niciun marker {selectedYear === null ? "în carieră" : "în acest an"}.
                    </span>
                  ) : null}
                </div>
              </div>
...
function RateLine({
  ...
  framework: DiscourseFramework;
  year: number | null;
  codedInWindow: number;
  primary: RateStats | null;
}) {
  const windowLabel = year === null ? "în întreaga carieră" : `în ${year}`;
  if (codedInWindow === 0) {
    return (
      <p className="text-sm leading-relaxed text-ink-45" data-tabular-nums="">
        Niciun discurs analizat {windowLabel} sub filtrul curent.
      </p>
    );
  }
  if (framework === "voice") {
    return (
      <p className="text-sm leading-relaxed text-ink-30" data-tabular-nums="">
        <span className={cn("label-mono mr-2", FRAMEWORK_FG.voice)}>Voce</span>
        {codedInWindow} discursuri analizate {windowLabel}, distribuite după voce dominantă.
      </p>
    );
  }
  ...
  return (
    <p className="text-sm leading-relaxed text-ink-30" data-tabular-nums="">
      ...
      <strong className="text-ink-16">{ge1Pct}%</strong> din {primary.total} discursuri analizate{" "}
      {windowLabel} au scor {label} ({primary.ge1} discursuri); <strong>{eq2Pct}%</strong> au scor{" "}
      {label2} ({primary.eq2}).
    </p>
  );
}
