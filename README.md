...
| `/cauta?q=…`                                                   | live          | hybrid speech search (BM25 + kNN/RRF), highlights, `noindex, follow`. Diacritic-insensitive: a query for `sosoaca` matches indexed `șoșoacă` via the `.folded` subfields on `text` / `agenda_title` / `speaker.name_search`.                                                          |
