...
The page combines a single `q` input with a progressively-disclosed filter panel — a `<details>` element labelled `Filtre · N` (count of active filters), expanded automatically when at least one filter is set. v1 ships six filters and a sort:
...
| Speech length | `length`     | `searchSpeeches.speechSizes` → `text_length` ranges | **multi-select** chip row (`XS · S · M · L · XL`) using the same vocabulary as the result-row word-count meter. URL: comma-separated, e.g. `?length=s,m`. The index stores `text_length` in characters, so the server filter uses calibrated character ranges as the query-time companion to the displayed word-count buckets.                                                                                                          |
...
The form is plain `<form action="/cauta" method="GET">` so it works without JS. [`<CautaFilterForm>`](../src/components/cauta/filter-form.tsx) is a thin `'use client'` wrapper that intercepts submission to (a) drop empty inputs, (b) collect every checked `year` checkbox and merge the `year-older` `<select>` into a single comma-joined `?year=` (sorted ascending for stable share-links), (c) collect every checked `length` checkbox into a comma-joined `?length=`, (d) `router.push` the resulting URL for soft navigation. Without JS, the browser submits the form as-is and the Zod parser accepts both comma-separated and repeated `length` values.
...
Invisible bot protection guards the three browser-initiated POST surfaces in the auth + OAuth-consent flow: **`/cont/intra`** (Google sign-in form), **`/cont`** (sign-out + DCR-client revoke), and **`/cont/consimt`** (OIDC consent accept/deny). These are the bot-attractive surfaces — account farming, MCP-client farming, consent-flow abuse — and they are 100% browser-driven, which is the only flow BotID can classify.

Wiring:

- `next.config.ts` wraps the export with `withBotId(withMDX(nextConfig))` from `botid/next/config` to install proxy rewrites at obfuscated paths (ad-blocker resilient).
- [`src/instrumentation-client.ts`](../src/instrumentation-client.ts) calls `initBotId({ protect: [...] })` with the three page URLs. **Server actions submit to the page URL the form was rendered on**, not the action file path, so the protect list is page URLs.
- Each of the five guarded server actions (`signInWithGoogle`, `signOutAction`, `revokeClientAction`, `acceptConsentAction`, `denyConsentAction`) calls `await checkBotId()` first and hard-throws on `isBot`. No soft-degrade: the population of legitimate users whose browser fails the BotID challenge is small and acceptable collateral for these specific endpoints.

Surfaces deliberately **not** protected: `/api/auth/[...all]` (mostly server-to-server Better Auth calls + GET Google callback; the OAuth POSTs that do exist are from MCP clients, no JS), `/mcp/server` (bearer-token server-to-server — protecting it would break Claude/Cursor/Codex instantly), `/api/search/persons` (GET autocomplete, false-positive risk outweighs scrape risk), and every RSC page (BotID is for POSTs). If you reach to protect more than the three `/cont*` POSTs, re-read [`docs/_session-handoff-2026-05-10-botid.md`](./_session-handoff-2026-05-10-botid.md) — the path list in `instrumentation-client.ts` is the contract, and missing client-side init = `checkBotId()` always says bot.

Mode: Basic at launch (free). Deep Analysis ($1/1k `checkBotId()` calls, Pro+) deferred until firewall observability shows actual pressure. No env vars — credentials are derived from the Vercel deployment itself. Local `bun run dev` always returns `isBot: false`; preview deployments issue real verdicts.
