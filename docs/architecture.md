...
- **Rate limits** ([`src/lib/ratelimit.ts`](../src/lib/ratelimit.ts)) — Upstash sliding window across two axes (per-IP, per-user). 30/min general + 6/min heavy on each axis; both must clear. The wrapper sniffs the JSON-RPC body to identify heavy calls without disturbing the downstream handler. No-op when Upstash creds aren't set (dev only — production sets both).
